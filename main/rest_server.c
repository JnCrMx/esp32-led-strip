...
static esp_err_t color_get_handler(httpd_req_t *req)
{
    ...
    const char *color_info = cJSON_Print(root);
    httpd_resp_sendstr(req, color_info);

    free((void *)color_info);
    cJSON_Delete(root);
    return ESP_OK;
}
...
esp_err_t start_rest_server(const char *base_path)
{
    ...
    httpd_register_uri_handler(server, &color_post_uri);
